...
import com.ruyuan2020.little.project.rocketmq.api.order.dto.OrderItemDTO;
import com.ruyuan2020.little.project.rocketmq.api.order.enums.OrderBusinessErrorCodeEnum;
...
import com.ruyuan2020.little.project.rocketmq.common.constants.StringPoolConstant;
...
public class OrderServiceImpl implements OrderService {
    ...
    private void saveOrderInfo(OrderInfoDTO orderInfoDTO) {
        ...
        mysqlRequestDTO.setSql(
            "insert into " + " t_shop_order_goods" + "(" + "thumb, " + "beid, " + "orderid, " + "goodsId, " + "title, "
                + "price, " + "total, " + "order_dates, " + "description, " + "createtime " + ")" + "values( " + "?, "
                + "?, " + "?, " + "?, " + "?, " + "?, " + "?, " + "?, " + "?, " + "? " + ")");
        ...
        LOGGER.info("end save orderItem param:{}, response:{}", JSON.toJSONString(mysqlRequestDTO),
    ...
    private void saveOrderItemInfo(OrderInfoDTO orderInfoDTO) {
        ...
        String phoneNumber = orderInfoDTO.getPhoneNumber();
        // 订单号
        orderInfoDTO.setOrderNo(
            UUID.randomUUID().toString().replace(StringPoolConstant.DASH, StringPoolConstant.EMPTY));
        orderInfoDTO.setStatus(OrderStatusEnum.WAITING_FOR_PAY.getStatus());
        orderInfoDTO.setPhoneNumber(phoneNumber);
        // 房间数据
        CommonResponse<HotelRoom> commonResponse = hotelRoomService.getRoomById(orderInfoDTO.getRoomId().longValue(),
            phoneNumber);
        ...
        MysqlRequestDTO mysqlRequestDTO = new MysqlRequestDTO();
        mysqlRequestDTO.setSql(
            "insert into " + " t_shop_order" + "(" + "beid, " + "openid, " + "ordersn, " + "price, " + "status, "
                + "remark, " + "address_realname, " + "address_mobile, " + "desk_num, " + "goods_total_price, "
                + "createtime, " + "updatetime, " + "coupon_id, " + "coupon_money, " + "uid " + ") " + "values( "
                + "?, " + "?, " + "?, " + "?, " + "?, " + "?, " + "?, " + "?, " + "?, " + "?, " + "?, " + "?, " + "?, "
                + "?, " + "?" + ")");
        ...
        LOGGER.info("end save order param:{}, response:{}", JSON.toJSONString(mysqlRequestDTO),
            JSON.toJSONString(insertOrderResponse));
    ...
    public CommonResponse cancelOrder(String orderNo, String phoneNumber) {
        ...
        if (!Objects.equals(status, OrderStatusEnum.WAITING_FOR_PAY.getStatus())) {
    ...
    public void informFinishedOrder(String orderNo, String phoneNumber) {
        ...
        try {
            ...
            LOGGER.error("finished order send half message fail error:{}",e.getMessage());
        ...
        // 下发优惠券
            ...
            // // 发送退房成功通知
            // orderEventInformManager.informOrderFinishEvent(this.getOrderInfo(orderNo, phoneNumber));
        // } catch (Exception e) {
        //     // TODO  保证主要权益的发放需要重试
    ...
    private void updateOrderStatusAndCancelTime(String orderNo, long cancelTime, String phoneNumber) {
        MysqlRequestDTO mysqlRequestDTO = new MysqlRequestDTO();
        mysqlRequestDTO.setSql(
            "update t_shop_order set status = ?,cancel_time = ?,coupon_id = null,coupon_money = null where ordersn = ?");
        List<Object> params = new ArrayList<>();
        ...
        LOGGER.info("start update order status param:{}", JSON.toJSONString(params));
        CommonResponse<Integer> response = mysqlApi.update(mysqlRequestDTO);
        LOGGER.info("end update order status param:{}, response:{}", JSON.toJSONString(params),
    ...
    private OrderInfoDTO getOrderInfo(String orderNo, String phoneNumber) {
        ...
        orderInfoRequestDTO.setSql(
            "select  " + "id, " + "address_realname , " + "status , " + "remark , " + "createtime , " + "uid , "
                + "beid , " + "address_mobile, " + "coupon_id " + " from t_shop_order " + " where " + "ordersn = ?");
        List<Object> params = new ArrayList<>();
        params.add(orderNo);
        ...
        LOGGER.info("end query order info param:{}, response:{}", JSON.toJSONString(orderInfoRequestDTO),
            JSON.toJSONString(orderInfoResponse));
    ...
    private OrderItemDTO getOrderItem(Integer orderId, String phoneNumber) {
        ...
        orderItemRequestDTO.setSql(
            "select  " + "goodsid , " + "title , " + "price , " + "total  " + " from t_shop_order_goods "
                + " where orderid = ?");
        ...
        LOGGER.info("end query order item param:{}, response:{}", JSON.toJSONString(orderItemRequestDTO),
            JSON.toJSONString(orderItemResponse));
